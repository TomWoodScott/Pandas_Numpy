...
df = pd.read_csv('../data1/fcc-forum-pageviews.csv', index_col=0, parse_dates=True)
...
def draw_bar_plot():
    ...
    # Want to group the data years and then the months as subsets of each year,
    # from these groups we want the mean value, this will give the average views per month
    df_bar = df_bar.groupby(["year", "month"])["value"].mean()

    # df.unstack() Returns a DataFrame having a new level of column labels whose inner-most level consists of the pivoted index labels.
    df_bar = df_bar.unstack()

    # Draw bar plot
    fig = df_bar.plot.bar(figsize=(16,8)).figure
    plt.xlabel("Years")
...
def draw_box_plot():
    ...
    # need to start with jan...
    df_box["month_in_order"] = df_box["date"].dt.month
    df_box = df_box.sort_values("month_in_order")


    # Draw box plots (using Seaborn)

    fig, ax = plt.subplots(nrows=1, ncols=2, figsize=(16,7))
    # we can refer to specific column by ax[ncol_index]
